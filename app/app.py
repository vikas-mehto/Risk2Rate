...
if predict_btn:
    ...
    c1.metric(
        "Annual Premium",
        f"₹ {adjusted_prediction:,.0f}"
    )

    c2.metric(
        "Monthly Premium",
        f"₹ {adjusted_prediction / 12:,.0f}"
    )
    ...
    if smoker == "Yes":
        reasons.append("Smoking increases health risk.")
    ...
    if pre_existing_disease == "Yes":
        reasons.append("Pre-existing disease increases claims.")
    ...
    if smoker == "Yes":
        recommendations.append(
            "Quitting smoking may reduce future premiums."
        )

    if sum_insured > 500000:
        recommendations.append(
            "Consider lower sum insured if appropriate."
        )

    if co_payment_percent == 0:
        recommendations.append(
            "Choosing co-payment can reduce premium."
        )
    ...
    for insurer, factor in insurer_adjustment.items():
        ...
        comparison.append({
            ...
            "Estimated Annual Premium (₹)": round(
                adjusted_prediction * factor,
                0
            )
    ...
    pdf.drawString(
        50,
        800,
        "Health Insurance Premium Prediction Report"
    )

    pdf.drawString(
        50,
        770,
        f"Annual Premium: ₹ {adjusted_prediction:,.0f}"
    )

    pdf.drawString(
        50,
        750,
        f"Monthly Premium: ₹ {adjusted_prediction/12:,.0f}"
    )

    pdf.drawString(
        50,
        730,
        f"Smoking Status: {smoker}"
    )
